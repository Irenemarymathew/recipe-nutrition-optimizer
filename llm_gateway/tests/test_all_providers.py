#!/usr/bin/env python3
"""Per-provider matrix test for llm_gateway.

Tests A (basic), B (tools), C (structured), D (caching), E (reasoning)
against Gemini, Nvidia, and Groq. Prints a matrix at the end.

Assumes the gateway is running at http://localhost:8100 (env LLM_GATEWAY_URL to override).
"""
from __future__ import annotations
import os, sys, json, time, httpx

URL = os.getenv("LLM_GATEWAY_URL", "http://localhost:8100")
PROVIDERS = ["g", "n", "gr"]
PROVIDER_NAMES = {"g": "gemini", "n": "nvidia", "gr": "groq"}
...
def main():
    ...
    matrix = {}
    all_details = {}
    print("Running all 3 providers in parallel...\n", flush=True)
